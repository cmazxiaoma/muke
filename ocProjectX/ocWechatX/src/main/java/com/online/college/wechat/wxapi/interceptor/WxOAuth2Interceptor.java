            ...
            if (StringUtils.isNotBlank(code)) {
                try {
                    ...
                    if (!StringUtils.isBlank(openid)) {
                        ...
                        // 缓存当前用户个人信息
                        OAuthUserInfo oAuthUserInfo = WxApiClient.getOAuthUserInfo(openid);
                        WxMemoryCacheClient.setOAuthUserInfo(sessionid, oAuthUserInfo);
                ...
                // 请求code的回调url
                String redirectUrl = HttpUtil.getRequestFullUri(request);
                String state = OAuth2RequestParamHelper.prepareState(request);

                /**
                 * 请求微信OAuth认证接口 微信服务器会回调：redirectUrl （即当前拦截的url）
                 */
                String url = WxApi.getOAuthCodeUrl(mpAccount.getAppid(), redirectUrl, OAuthScope.Userinfo.toString(),
                        state);
